...
from lib.midnight_magnates import vocab
from ._contract import Finding, GateInputError, load_json, run_cli

# The primitive this gate keys off. Sourced from the canonical vocabulary (not a
# bare literal) so a rename of the storyboard primitive is caught here rather
# than silently disabling the gate. The assert pins the exact member name: if
# 'migration_arrow' is ever renamed in vocab.PRIMITIVES this import-time check
# fails loudly instead of letting every travel beat pass un-gated.
MIGRATION_ARROW = "migration_arrow"
assert MIGRATION_ARROW in vocab.PRIMITIVES, (
    "qa_migration_icon: {0!r} is no longer a member of vocab.PRIMITIVES — "
    "the primitive was renamed; update MIGRATION_ARROW.".format(MIGRATION_ARROW)
)
            ...
            for beat in beats:
                ...
                for ai, la in enumerate(arrows):
                    ...
                    where = "{0} :: {1} :: {2} :: {3}[{4}]".format(
                        cid, phase_id, bid, MIGRATION_ARROW, ai)
                    if rationale is not None:
                        findings.append(Finding(
                            "warn", "iconless_route_documented",
                            "{0} on a travel line (VO/params name {1!r}) has no "
                            "character_icon, but a rationale is documented: {2!r}".format(
                                MIGRATION_ARROW, method, rationale),
                            where=where,
                        ))
                        continue
                    findings.append(Finding(
                        "fail", "migration_arrow_missing_icon",
                        "the VO/params frame this route as travel ({0!r}) but the {1} "
                        ...
                        "params.no_icon_rationale to document why it is icon-less.".format(
                            method, MIGRATION_ARROW),
                        where=where,
                    ))
    return findings
