...
# Canonical source: vocab.PERSISTENT_KINDS == the PERSISTENT_UI family PLUS
# "paper_texture" (which the family files under ATMOSPHERIC). This is NOT
# vocab.PRIMITIVE_FAMILIES["PERSISTENT_UI"], which omits paper_texture.
PERSISTENT_KINDS = vocab.PERSISTENT_KINDS
...
def _master_end(project_dir: Path, cues: List[dict]) -> Optional[float]:
    ...
    ends = [
        float(c.get("end_s", c.get("t_out")))
        for c in cues
        if isinstance(c.get("end_s", c.get("t_out")), (int, float))
    ]
...
def check(project_dir: Path, args: Namespace) -> List[Finding]:
    ...
    for i, cue in enumerate(cues):
        cid = cue.get("id", f"#{i}")
        # Accept t_in/t_out as aliases (overlay cuelist uses them; maps uses start_s/end_s).
        start, end = cue.get("start_s", cue.get("t_in")), cue.get("end_s", cue.get("t_out"))

        if not isinstance(start, (int, float)) or not isinstance(end, (int, float)):
            findings.append(Finding(
                "fail", "missing_timing",
                "cue needs numeric start_s/end_s (or t_in/t_out) to verify its lifecycle",
                where=cid,
            ))
            continue
        start = float(start)
