...
hit the frame), music/ambient are looser beds that can ease in. An *impulsive*
sfx (a transient that fires on a single frame — flagged by category
"sfx_accent" or an explicit ``impulsive: true``) gets an even tighter leash than
a generic/sustained sfx: a clock tick or gunshot a tenth of a second late reads
as a clear sync miss where a slowly-swelling whoosh would not.

The compare is on the cue's ONSET, not its declared ``t_in``: a cue may carry a
``transient`` head (silence/attack before the audible peak) and an ``offset_ms``
nudge, so the moment the listener actually HEARS is
``t_in + transient + offset_ms/1000`` — that is what must land on the anchor.
...
Rule (fail):
  ...
  * For ANY cue that HAS an anchor_phrase: resolve it as a contiguous
    ...
    fallback_absolute_s.
  * event_t = t_in + (transient or 0) + (offset_ms or 0)/1000. Then if
    abs(event_t - expected_t) > budget -> `drift_exceeds_budget` (reports the
    delta + the budget). The budget is the IMPULSIVE bucket for an impulsive
    cue, else the per-category bucket.
...
Shapes (only the fields this gate reads):
    sound_cuelist = {"cues": [
        {"id", "category" ("music"|"sfx"|"ambient"|"sfx_accent"), "asset",
         "t_in", "t_out", "anchor_phrase"?, "fallback_absolute_s"?,
         "transient"?, "offset_ms"?, "impulsive"?}, ...]}
...
# music/ambient beds may ease in, so they get a looser leash. An impulsive sfx
# (transient that fires on a single frame) gets the tightest bucket of all,
# distinct from a generic/sustained sfx.
BUDGET = {"impulsive": 0.08, "sfx": 0.15, "music": 0.5, "ambient": 2.0}
...
def _num_or_zero(v) -> float:
    """A numeric offset/transient field, defaulting to 0.0 when absent/non-numeric."""
    return float(v) if _is_number(v) else 0.0


def _is_impulsive(cue: dict, category: str) -> bool:
    """A cue is impulsive if explicitly flagged or it's an accent (single-frame
    transient). Impulsive cues get the tightest drift bucket."""
    return cue.get("impulsive") is True or category == "sfx_accent"
...
def check(project_dir: Path, args: Namespace) -> List[Finding]:
    ...
    for i, cue in enumerate(cues):
        ...
        # An impulsive accent gets the tightest bucket regardless of how its
        # category resolves; otherwise fall back to the per-category budget.
        impulsive = _is_impulsive(cue, category)
        budget = BUDGET["impulsive"] if impulsive else BUDGET.get(category)
        if budget is None:
            findings.append(Finding(
        ...
        # Compare the cue's ONSET — t_in plus any transient attack/silence head
        # plus an offset_ms nudge — to the anchor, not the raw t_in.
        event_t = float(t_in) + _num_or_zero(cue.get("transient"))
        event_t += _num_or_zero(cue.get("offset_ms")) / 1000.0
        delta = abs(event_t - expected_t)
        if delta > budget:
            findings.append(Finding(
                "fail", "drift_exceeds_budget",
                "onset {0:.2f}s is {1:.2f}s off anchor at {2:.2f}s ({3} "
                "budget {4:.2f}s)".format(
                    event_t, delta, expected_t,
                    "impulsive" if impulsive else category, budget),
                where=cid))
    return findings
