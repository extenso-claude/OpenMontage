...
def _referenced_asset_ids(cuelist: dict) -> Set[str]:
    ...
    for cue in cues:
        ...
        # Assets are also referenced inside the cue sub-structures the compiler
        # emits, NOT only at the top level: a migration_arrow's traveling
        # rider/transportation sprite is carried in cue.route_icon.asset_id (so the
        # icon can ride the polyline), and the all_presidents_pins/pins_px roster may
        # carry per-pin asset_ids. These are legitimately USED assets — collect them
        # so the closure check doesn't flag a wired rider sprite "sourced-but-unused"
        # (which contradicted qa_migration_icon passing the same cue).
        ric = cue.get("route_icon")
        if isinstance(ric, dict):
            ric_aid = ric.get("asset_id")
            if isinstance(ric_aid, str) and ric_aid.strip():
                referenced.add(ric_aid)
        for pin in (cue.get("pins_px") or []):
            if isinstance(pin, dict):
                pin_aid = pin.get("asset_id")
                if isinstance(pin_aid, str) and pin_aid.strip():
                    referenced.add(pin_aid)
    return referenced
