...
CLI:
  python -m lib.midnight_magnates.runner run-gates  --pipeline P --project D [--stage S]
  python -m lib.midnight_magnates.runner run-stage  --pipeline P --project D --stage S
  python -m lib.midnight_magnates.runner check-lock --pipeline P --project D
"""
...
RUNNER_SIGNATURE = f"midnight_magnates.runner@{__version__}"
GATE_TIMEOUT_S = 600
...
_HASH_CANDIDATES = [
    ...
    "artifacts/sound_cuelist.json",        # MM: an SFX re-time must re-stale a green lock
    "artifacts/sound_design_report.json",
    "artifacts/asset_manifest.json",
    "artifacts/face_manifest.json",        # MM rule 4: emotional-face source index
    "artifacts/render_modes.json",         # MM rule 2: per-shot 2d/3d + render_mode flag
    "artifacts/shot_status.json",          # MM: self-QA status (a flip must re-stale the lock)
    "artifacts/theme.json",
    "artifacts/geography.json",            # optional for MM (absent on no-map videos; skipped)
    "artifacts/canonical_names.json",
    "artifacts/whisper/full.json",         # MM: the VO timing spine every drift gate resolves against
    # sleep-network-overlay artifacts (the overlay-specific gates read these):
    "artifacts/source_baked_elements.json",  # qa_source_collision catalog — a re-catalog re-stales the lock
    "artifacts/avatar_manifest.json",         # qa_avatar_sync — an avatar re-time re-stales the lock
...
def _compute_input_hashes(project_dir: Path) -> dict:
    ...
    # sleep-network-overlay: composite review-swarm round artifacts — a new round must
    # re-stale a green lock (the auditable-rounds enforcement in composite-review-swarm.md).
    rel_paths += [
        str(p.relative_to(project_dir).as_posix())
        for p in sorted((project_dir / "artifacts").glob("review_round_*.json"))
    ]
...
def run_gates(
    ...
    validate_artifact("midnight_magnates_qa_report", report)
...
def _main(argv: Optional[List[str]] = None) -> int:
    ap = argparse.ArgumentParser(prog="midnight_magnates.runner")
    sub = ap.add_subparsers(dest="cmd", required=True)
    for name in ("run-gates", "run-stage", "check-lock"):
        sp = sub.add_parser(name)
        sp.add_argument("--pipeline", default="midnight-magnates-doc")
