...
needs (delegating the pixel work to lib.midnight_magnates.cutout). The
...
# Character primitives — the CARD_PRIMS members whose name starts with
# "character_card". Single-sourced from vocab (was a hand-copy of
# ("character_card", "character_card_pop")).
CHARACTER_PRIMITIVES = vocab.CHARACTER_CARD_PRIMS
...
# not floating on empty parchment: FULLFRAME_PRIMS that carry scene imagery, plus
# map_sprite (a placed scene illustration on the map) and parallax_layers.
# Single-sourced from vocab (was an inexact hand-mirror).
SCENE_ANCHOR_PRIMITIVES = vocab.SCENE_ANCHOR_PRIMS
...
def resolve_hero_cutout(
    ...
    *,
    ...
    lib.midnight_magnates.cutout.cutout (rembg / U2Net), so there is exactly
